...
- `useAuthErrorMessage()` (`app/composables/useAuthErrorMessage.ts`) takes the whole failure — an `AuthActionError` from `useSignIn()`, a raw `?error=` query value, or a repeated query array — and tries `auth.errors.<CODE>` before `auth.errors.<MESSAGE>`, falling back to the message text. Better Auth's `code` is the stable translation key; `message` is only a fallback for failures that carry no code
...
- `config/i18n-empty-placeholders.ts` provides the Vite plugin (registered from the `vite:extendConfig` hook in `nuxt.config.ts`) that drops empty leaves from `i18n/locales/**/*.json` at build time, so vue-i18n falls back to `en-US` instead of rendering `""`. Locale _types_ are still generated from the on-disk files, so stripped keys stay valid in `$t()` call sites
