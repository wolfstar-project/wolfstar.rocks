...
By default Vitest runs test files in [multiple processes](/guide/parallelism) using `node:child_process`, allowing tests to run simultaneously. If you want to speed up your test suite even further, consider enabling `--pool=threads` to run tests using `node:worker_threads` (beware that some packages might not work with this setup).
...
You can run benchmark tests with [`bench`](/api/test#bench) function via Tinybench to compare performance results.
...
You can [write tests](/guide/testing-types) to catch type regressions. Vitest comes with `expect-type` package to provide you with a similar and easy to understand API.
...
By default, Vitest catches and reports all unhandled rejections, uncaught exceptions (in Node.js) and error events (in the [browser](/guide/browser/)).
