...
import Reactions from "~/components/guild/settings/filter/Reactions.vue";
...
const mockGuildSettings = ref<GuildData | undefined>(
	createMockGuildData("123456789012345678", {
		...
		selfmodReactionsSoftAction: 0b010,
		selfmodReactionsHardAction: 5,
		selfmodReactionsHardActionDuration: 86400000n,
		selfmodReactionsThresholdMaximum: 8,
		selfmodReactionsThresholdDuration: 15_000,
	}),
);
...
function getSetupState(wrapper: Awaited<ReturnType<typeof mountSuspended>>) {
	return (wrapper.vm.$ as any).setupState as {
		state: {
			...
			selfmodReactionsThresholdDurationSeconds: number;
...
describe("reaction filter guild settings", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it("renders filter toggle switches", async () => {
		const wrapper = await mountSuspended(Reactions);

		await nextTick();
		await nextTick();

		expect(getSwitch(wrapper, "Toggle reaction filter").exists()).toBeTruthy();
	...
	it("initializes threshold duration in seconds from milliseconds", async () => {
		...
		expect(state.selfmodReactionsThresholdDurationSeconds).toBe(15);
	});
	...
	it("converts hard action duration from BigInt to number", async () => {
		...
		expect(state.hardActionDurationMs).toBe(86400000);
	});
	...
	it("shows loading skeleton when guildSettings is undefined", async () => {
		...
		expect(wrapper.find('form[aria-label="Reaction filter settings form"]').exists()).toBe(
			false,
		);
