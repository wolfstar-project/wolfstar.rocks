<template>
	<div class="space-y-6">
		...
		<GuildSettingsForm
			...
			class="space-y-6"
			:aria-label="ts('guild_settings.filter.word.form_aria')"
			@error="onError"
		>
			<!-- Section 1: Word Filter -->
			<GuildSettingsSection :title="ts('guild_settings.filter.word.title')">
				<div class="space-y-3">
					<UFormField name="selfmodFilterEnabled">
						<div class="flex items-start gap-3">
							<USwitch
								...
								class="mt-0.5"
								:aria-label="ts('guild_settings.filter.word.toggle')"
							/>
							<div>
								<p class="text-sm leading-none font-medium">
									{{
										ts("guild_settings.filter.word.filter_label", {
								...
								<p class="mt-1 text-xs text-muted">
									{{ ts("guild_settings.filter.word.toggle_help") }}
								</p>
							</div>
						</div>
					</UFormField>
			...
			<GuildSettingsSection :title="ts('guild_settings.filter.word.filtered_words')">
				<div class="space-y-4">
					<form class="flex gap-2" @submit.prevent="addWord">
						<UInput
							v-model="newWord"
							:placeholder="ts('guild_settings.filter.word.add_placeholder')"
							class="w-64"
							:aria-label="ts('guild_settings.filter.word.add_word')"
						/>
					...
					<div v-if="state.selfmodFilterRaw.length > 0" class="flex flex-wrap gap-2">
						...
						>
							...
							<button
								...
								:aria-label="ts('guild_settings.filter.word.remove_word', { word })"
...
async function onError(event: FormErrorEvent) {
	...
	toast.add({
		color: "error",
		description: ts("guild_settings.filter.word.save_failed", {
