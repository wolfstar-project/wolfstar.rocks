...
export type DeepEqualPath = ReadonlyArray<DeepEqualPathSegment>;
...
export function isDeepEqual(a: unknown, b: unknown, options: IsDeepEqualOptions = {}): boolean {
  ...
  const inner = (left: unknown, right: unknown, path: DeepEqualPath, report: boolean): boolean => {
    ...
    if (custom !== undefined) {
      if (!custom)
        reportDifference({ path, left, right }, report);
      return custom;
    }
    ...
    if (leftObj instanceof Map) {
      ...
      const rightEntries = Array.from(rightObj.entries());
      ...
      for (const [leftKey, leftValue] of leftObj.entries()) {
        ...
        if (keyCandidates.length === 0) {
          reportDifference({ path: [...path, "<mapKey>"], left: leftKey, right: undefined }, report);
        }
      ...
      if (!equal && mode !== "first") {
        for (let i = 0; i < rightEntries.length; i++) {
          if (used.has(i))
            continue;
          const [rightKey] = rightEntries[i]!;
          reportDifference({ path: [...path, "<mapKey>"], left: undefined, right: rightKey }, report);
        }
      }
    ...
    if (leftObj instanceof Set) {
      ...
      const rightValues = Array.from(rightObj.values());
      const used = new Set<number>();
      ...
      for (const leftValue of leftObj.values()) {
        ...
        equal = false;
        reportDifference({ path: [...path, "<setValue>"], left: leftValue, right: undefined }, report);
      ...
      if (!equal && mode !== "first") {
        for (let i = 0; i < rightValues.length; i++) {
          if (used.has(i))
            continue;
          reportDifference({ path: [...path, "<setValue>"], left: undefined, right: rightValues[i] }, report);
        }
      }
    ...
    for (const key of leftKeys) {
      if (!Object.prototype.hasOwnProperty.call(rightObj, key)) {
        reportDifference({ path: [...path, key], left: leftRecord[key], right: undefined }, report);
        equal = false;
        if (mode === "first")
          return cacheAndReturn(false);
        continue;
