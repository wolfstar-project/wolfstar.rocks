<template>
	<div class="space-y-6">
		...
		<GuildSettingsForm
			...
			class="space-y-6"
			:aria-label="ts('guild_settings.filter.messages.form_aria')"
			@error="onError"
		>
			<!-- Section 1: Message Duplication Filter -->
			<GuildSettingsSection :title="ts('guild_settings.filter.messages.title')">
				<div class="space-y-3">
					<UFormField name="selfmodMessagesEnabled">
						<div class="flex items-start gap-3">
							<USwitch
								...
								class="mt-0.5"
								:aria-label="ts('guild_settings.filter.messages.toggle')"
							/>
							<div>
								<p class="text-sm leading-none font-medium">
									{{
										ts("guild_settings.filter.messages.filter_label", {
								...
								<p class="mt-1 text-xs text-muted">
									{{ ts("guild_settings.filter.messages.toggle_help") }}
								</p>
							</div>
						</div>
					</UFormField>
...
async function onError(event: FormErrorEvent) {
	...
	toast.add({
		color: "error",
		description: ts("guild_settings.filter.messages.save_failed", {
