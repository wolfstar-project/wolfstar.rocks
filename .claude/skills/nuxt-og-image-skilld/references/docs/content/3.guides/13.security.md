...
For full protection, we recommend combining URL signing with a **web application firewall** (WAF) or rate limiting on the `/_og/` path prefix. Services like Cloudflare, AWS WAF, or your hosting provider's built-in rate limiting can add an additional layer of defense.
