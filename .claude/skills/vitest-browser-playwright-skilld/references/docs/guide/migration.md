...
- Vitest no longer injects `__vitest_executor` into every [test runner](/api/advanced/runner). Instead, it injects `moduleRunner` which is an instance of `ModuleRunner`
- `vitest/execute` entry point was removed. It was always meant to be internal
- [Custom environments](/guide/environment) no longer need to provide a `transformMode` property. Instead, provide `viteEnvironment`. If it is not provided, Vitest will use the environment name to transform files on the server (see `server.environments`)
...
Jest has their globals API enabled by default. Vitest does not. You can either enable globals via [the `globals` configuration setting](/config/globals) or update your code to use imports from the `vitest` module instead.

If you decide to keep globals disabled, be aware that common libraries like `testing-library` will not run auto DOM cleanup.

### `mock.mockReset`

Jest's `mockReset` replaces the mock implementation with an
...
If you want to modify the object, you will use replaceProperty API in Jest, you can use [`vi.stubEnv`](/api/vi#vi-stubenv) or [`vi.spyOn`](/api/vi#vi-spyon) to do the same also in Vitest.
...
This is not a Jest-specific feature, but if you previously were using Jest with vue-cli preset, you will need to install `jest-serializer-vue` package, and specify it in [`snapshotSerializers`](/config/snapshotserializers):
