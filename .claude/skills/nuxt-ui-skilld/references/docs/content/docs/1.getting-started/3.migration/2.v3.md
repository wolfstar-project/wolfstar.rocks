...
- The `Table` component now uses TanStack Table{target="_blank" rel="noopener"} under the hood. The `rows` prop has been renamed to `data`:
...
::note
The `Carousel` component now uses Embla Carousel{target="_blank" rel="noopener"} under the hood.
::
