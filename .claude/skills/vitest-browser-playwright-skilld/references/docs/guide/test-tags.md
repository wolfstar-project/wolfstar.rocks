...
- `not` or `*` to match any number of characters (0 or more)
- `()` to group expressions and override precedence

The parser follows standard operator precedence: `not`/`and`/`&&`, then `or`/`||`. Use parentheses to override default precedence.
...
To exclude tests with a specific tag, add an exclamation mark (```shell
