...
::: warning IMPORTANT
Vitest spies inherit implementation's `length` property when initialized, but it doesn't override it if the implementation was changed later:
...
Shorthand methods like `mockReturnValue`, `mockReturnValueOnce`, `mockResolvedValue` and others cannot be used on a mocked class. Class constructors have unintuitive behaviour regarding the return value:
