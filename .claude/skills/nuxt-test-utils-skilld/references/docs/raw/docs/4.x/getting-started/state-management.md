...
[`useState`](/docs/4.x/api/composables/use-state) is an SSR-friendly `ref` replacement. Its value will be preserved after server-side rendering (during client-side hydration) and shared across all components using a unique key.
...
Make sure to install the Pinia module with `npx nuxt module add pinia` or follow the module's installation steps.
