...
By using tree shaking it is possible to shave
...
@sentry/opentelemetry in the Readme.
...
bumps the internally used typescript version from 3.8.x to 4.9.x.
...
you think. Respond on GitHub or write to us at
...
We've added a new middleware for trpc that automatically adds TRPC information to Sentry
...
Special shoutout to @Tofandel who helped fix a bug in Jest that was
...
This release adds the `source` field to
...
- [Updated CommonJS distributions to use ES6 by default](./MIGRATION.md#moving-to-es6-for-commonjs-files). If you need
  ...
  Babel to convert Sentry packages to ES5. (#5005)
...
- Start propogating `baggage` HTTP header alongside `sentry-trace` header to
