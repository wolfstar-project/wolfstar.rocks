...
describe.skip("useUser", () => {
  ...
  it("should cache data with fetchAt timestamp", async () => {
    ...
    const _result = useUser(mockUser);
  ...
  it("should transform data to include transformedGuilds and fetchAt", async () => {
    ...
    // Mock useCachedFetch to return the mock response
    vi.mock("#app/composables/useCachedFetch", () => ({
    ...
    expect(result.data.value?.transformedGuilds).toEqual(mockTransformedGuilds);

    vi.restoreAllMocks();
    vi.unmock("#app/composables/useCachedFetch");
  });
});
