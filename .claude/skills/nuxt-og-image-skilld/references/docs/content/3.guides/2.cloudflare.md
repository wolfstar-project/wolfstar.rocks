...
OG image generation requires loading font files at runtime. On Cloudflare, fonts are served as static assets through the `ASSETS` binding. Without this binding, the worker cannot access fonts and all text will be invisible.
...
The module caches rendered OG images in memory by default, which resets on every deploy. For persistent caching, use Cloudflare KV.
...
Cloudflare Browser Rendering enables runtime screenshots for `.browser.vue` templates. This is not needed for Takumi or Satori templates.
...
Sessions have a 60 second idle timeout, handled automatically by the module. See Cloudflare Browser Rendering limits for details.
