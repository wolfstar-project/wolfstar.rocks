...
- [snapshotFormat | Config](./config/snapshotformat.md): Format options for snapshot testing. These options configure the snapshot-specific formatting layer built on top of @vitest/pretty-format.
...
- [ARIA Snapshots | Guide](./guide/browser/aria-snapshots.md): ARIA snapshots let you test the accessibility structure of your pages. Instead of asserting against raw HTML or visual output, you assert against t...
...
- [Features | Guide](./guide/features.md): ::: tip
This page is a high-level overview of Vitest's capabilities. If you're new to Vitest, we recommend reading the Learn tutorial first for a h...
- [Test Filtering | Guide](./guide/filtering.md): As your test suite grows, running every test on every change becomes slow and distracting. If you're fixing a bug in a single module, you don't nee...
...
powered by
Vite.
- [Test Run Lifecycle | Guide](./guide/lifecycle.md): ::: tip
...
- [Mocking | Guide](./guide/mocking.md): ::: tip
...
Please, leave feedback regarding this feature in a GitHub Discussion.
:::
- [Parallelism | Guide](./guide/parallelism.md): Vitest has two levels of parallelism: it can run multiple test files at the same time, and within each file it can run multiple tests at the same t...
...
- [Snapshot | Guide](./guide/snapshot.md): ::: tip
...
Vitest is powered by Vite. While you do not need to know Vite to use Vitest, understanding Vite helps explain some of Vitest's unique a...
...
- [Testing Asynchronous Code | Guide](./guide/learn/async.md): JavaScript code frequently runs asynchronously. Whether you're fetching data, reading files, or waiting on timers, Vitest needs to know when the co...
- [Debugging Failing Tests | Guide](./guide/learn/debugging-tests.md): This page covers how to investigate test failures in Vitest: reading error output, isolating problems, identifying common causes, and using the ava...
- [Using Matchers | Guide](./guide/learn/matchers.md): Vitest uses expect with "matchers" to assert that values meet certain conditions. This page covers the matchers you'll use most often. For the comp...
- [Mock Functions | Guide](./guide/learn/mock-functions.md): When writing tests, you often need to replace a real function or module with a controlled version. This is called mocking. There are several reason...
- [Setup and Teardown | Guide](./guide/learn/setup-teardown.md): Often while writing tests, you need to do some work before tests run (initialize data, connect to a database, start a server) and clean up afterwar...
- [Snapshot Testing | Guide](./guide/learn/snapshots.md): Snapshot tests capture the output of a piece of code and save it to a file. On subsequent runs, the output is compared against the saved snapshot. ...
- [Testing in Practice | Guide](./guide/learn/testing-in-practice.md): The previous pages covered the Vitest API: assertions, mocking, snapshots, and test lifecycle hooks. This page focuses on applying those tools to r...
- [Writing Tests with AI | Guide](./guide/learn/writing-tests-with-ai.md): AI coding assistants can help you write tests faster, but the quality of the output depends heavily on what you put in. A vague prompt produces vag...
- [Writing Tests | Guide](./guide/learn/writing-tests.md): In the Getting Started guide, you installed Vitest and ran your first test. This page dives deeper into how to write and organize tests in Vitest.
