Generate SKILL.md section for "@prisma/client" v7.7.0.
...
| Types | `./references/pkg/default.d.ts` — **read this file directly** to verify exports |
...
- `./references/pkg-client/` (1 .md files)
...
Skip: obvious API usage, installation steps, general TypeScript/programming patterns not specific to this package, anything a developer would naturally write without reading the docs. Every item must be specific to @prisma/client — reject general programming advice that applies to any project.

Search: `skilld search "recommended" -p @prisma/client`, `skilld search "avoid" -p @prisma/client`

## Format

<format-example note="Illustrative structure only — replace placeholder names with real @prisma/client APIs">
```
## Best Practices

- Use @prisma/client's built-in `createX()`./references/docs/api.md#createx)
