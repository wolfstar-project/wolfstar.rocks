...
Babel to convert Sentry packages to ES5.
...
third-party CDNs like unpkg or jsDelivr will also not provide them.
...
the `baggage` HTTP header to outgoing requests if the instrumentation of requests is
...
- Remove deprecated `whitelistUrls` and `blacklistUrls` options from `Sentry.init`. They have been superseded by
  ...
  our docs page on inclusive language for more details.
...
- Remove `Backend` and port its functionality into `Client` (see
  #4911 and
  #4919). `Backend` was an unnecessary abstraction which is
...
in the SDK (see #3282): `TraceDirective` and `TraceModule`
...
PR that made this change. To migrate, see the following
example.
