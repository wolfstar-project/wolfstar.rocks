...
- Components read translations through `const { ts: t } = useI18n()`, not `t`. Micro's `t()` returns `CleanTranslation` (it can hand back an object or array for a non-leaf key); `ts()` is the string-safe variant with the same `(key, params, defaultValue)` signature, so aliasing it keeps every `t("…")` call site type-safe. `te()` is `has()`, and pluralization is `tc(key, count)` (`t(key, named, plural)` no longer exists)
