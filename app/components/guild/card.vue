	...
	>
		...
		<template v-if="!loading && guild">
			...
			<div class="flex flex-col items-center gap-2 md:hidden">
				...
				<NuxtLink v-else-if="guild.manageable" :to="guildAddURL(guild.id)" class="group relative" :aria-label="`Invite bot to ${guild.name}`">
					<guild-icon :guild variant="bare" size="lg" :show-status="true" />
					<div
				...
				<div v-else class="relative cursor-not-allowed opacity-60" role="img" :aria-label="`${guild.name} - No permission`">
					<guild-icon :guild variant="bare" size="lg" :show-status="true" />
				</div>
				<!-- Guild Name -->
			...
			<div class="hidden h-full w-full flex-col items-center gap-3 text-center md:flex">
				...
				<div class="flex items-center justify-center gap-4 text-xs text-base-content/60">
					<span class="flex items-center gap-1" title="Total members" aria-label="Total members">
						<UIcon name="heroicons:user-group" class="size-3 text-base-content/70" aria-hidden="true" />
				...
				<div class="mt-auto w-full pt-2">
					...
					>
						<UIcon name="heroicons:adjustments-horizontal" class="mr-1 inline size-3" aria-hidden="true" />
						Manage Server
					</NuxtLink>
					<NuxtLink
...
const approximatePresenceCount = computed(() => {
	...
	return !isNullOrUndefinedOrZero(guild.approximatePresenceCount) ? formatNumber(guild.approximatePresenceCount) : "N/A";
});
...
const approximateMemberCount = computed(() => {
	...
	return !isNullOrUndefinedOrZero(guild.approximateMemberCount) ? formatNumber(guild.approximateMemberCount) : "N/A";
});
</script>
