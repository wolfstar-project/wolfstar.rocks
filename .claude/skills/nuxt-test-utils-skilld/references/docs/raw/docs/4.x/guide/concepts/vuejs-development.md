...
Most applications need multiple pages and a way to navigate between them. This is called **routing**. Nuxt uses an [`app/pages/`](/docs/4.x/directory-structure/app/pages) directory and naming conventions to directly create routes mapped to your files using the official Vue Router library.
...
- Use auto-imported Reactivity functions from Vue and Nuxt [built-in composables](/docs/4.x/api/composables/use-async-data).
