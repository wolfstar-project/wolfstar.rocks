...
This provider requires Javascript runtime that's implemented on top of V8 engine, such as NodeJS, Deno or any Chromium based browsers such as Google Chrome.
...
-  In some cases can be slower than Istanbul, e.g. when loading lots of different modules. V8 does not support limiting coverage collection to specific modules.
-  There are some minor limitations set by V8 engine. See `ast-v8-to-istanbul` | Limitations.
...
Istanbul code coverage tooling has existed since 2012 and is very well battle-tested.
