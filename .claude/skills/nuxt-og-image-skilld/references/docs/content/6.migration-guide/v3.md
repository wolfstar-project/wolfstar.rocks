...
- `runtimeBrowser`, `runtimeSatori` - These are now handled by `compatibility`. If you intend to use the Browser renderer at runtime please
see [the Browser docs](/docs/og-image/renderers/browser) for more information.
...
and have a use case for it, please open an issue.
...
Please see the [Runtime Cache docs](/docs/og-image/guides/runtime-cache) for details on using the new `cacheMaxAgeSeconds`.
