...
- `server/middleware/oauth-callback.ts` + `server/utils/oauth-callback.ts` (`resolveOAuthProviderCallbackRedirect()`) redirect Discord's response at `/oauth/callback` to the better-auth callback path when a `state` query param is present; plain browser navigations to `/oauth/callback` (no `state`) fall through to the Vue callback page at `app/pages/oauth/callback.vue`. `nuxt.config.ts` sets `routeRules["/oauth/callback"].prerender: false` — a prerendered copy would bypass the middleware and strand the browser on the static callback page instead of forwarding Discord's response
