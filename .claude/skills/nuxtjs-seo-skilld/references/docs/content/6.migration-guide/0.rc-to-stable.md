...
- `useBreadcrumbItems()`{lang="ts"} composable
...
This composable didn't do anything in v4 as the module enables the robots meta tag by default. If you'd like to control the robot meta tag rule, use the [`useRobotsRule()`{lang="ts"}](/docs/robots/api/use-robots-rule) composable.
...
This component was a simple wrapper for `defineRobotMeta`, you should use [`useRobotsRule()`{lang="ts"}](/docs/robots/api/use-robots-rule) if you wish to control the robots rule.
