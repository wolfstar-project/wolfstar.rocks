...
Jump in the Discord and ask me directly.
...
Use each module's `defineXxxSchema()`{lang="ts"} function in your `content.config.ts`. This works the same way whether you installed `@nuxtjs/seo` or the modules individually. Only include schemas for the modules you use.
...
If the production build size is a concern, you can [disable the modules](/docs/nuxt-seo/guides/disabling-modules) you don't need.
