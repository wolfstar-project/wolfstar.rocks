...
::warning
Make sure to wrap your app with the [`App`](/docs/components/app) component which uses our `Toaster` component which uses the `ToastProvider` component from Reka UI.
::
...
Set the `toaster.expand` prop to `false` on the [App](/docs/components/app#props) component to display stacked toasts (inspired by Sonner).
...
Use the `h()` render function in the `title` or `description` fields to render HTML elements or Vue components with custom styling.
