...
At the moment, Vitest ignores the [test.cache.dir](/config/cache) or cacheDir options completely and creates a separate folder.
...
- no `plugins`: plugins are not applied because there is no transformation phase, use customization hooks via [`execArgv`](/config/execargv) instead
...
- import a custom loader via `execArgv` flag
