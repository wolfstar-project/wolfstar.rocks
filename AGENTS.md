...
- `/oauth/login` (aliased at `/login`) starts sign-in in `onMounted` through `useAuthClient()?.signIn.social`, not `useSignIn("social")`: the action handle infers its `provider` union from a local server auth config, and with none present it types `provider` as `never`, so the call fails typecheck. The page keeps the same UX by tracking the failure itself — `signIn.social` reports it in the result rather than throwing — and renders a retry panel instead of an endless spinner. Its `callbackURL` / `errorCallbackURL` must be **absolute** frontend URLs (built from `window.location.origin`), or Better Auth resolves them against the bot API origin and the redirect lands on the bot
