<template>
	<div>
		<h1 class="sr-only">{{ ts("auth.oauth.guild_sr_title") }}</h1>
		<h2 class="sr-only">{{ ts("auth.oauth.guild_sr_status") }}</h2>
		<template v-if="!guildId">
			<OauthStatusPanel
				tone="error"
				:title="ts('auth.oauth.guild_not_found_title')"
		...
		<ClientOnly v-else>
			<template v-if="error">
				<OauthStatusPanel
					tone="error"
					:title="ts('auth.oauth.guild_setup_failed_title')"
				...
				>
					...
					<template #actions>
						<UButton to="/login" size="sm" variant="outline" class="w-full sm:w-auto">
							{{ ts("auth.oauth.guild_return_login") }}
						</UButton>
					</template>
				</OauthStatusPanel>
			</template>
			<template v-else>
				<OauthStatusPanel
					tone="info"
					loading
					:title="ts('auth.oauth.guild_redirecting_title')"
					icon="ph:discord-logo-fill"
				>
					<template #description>
						{{ ts("auth.oauth.guild_redirecting_description") }}
					</template>
				</OauthStatusPanel>
			</template>
		</ClientOnly>
...
async function navigateToGuild() {
	if (!guildId.value) {
		throw createError({ status: 400, statusText: ts("auth.oauth.guild_id_required") });
	}
...
useSeoMeta({
	ogDescription: ts("auth.oauth.guild_seo_og_description"),
	ogTitle: ts("auth.oauth.guild_seo_og_title"),
	robots: { none: true },
	title: ts("auth.oauth.guild_seo_title"),
});
</script>
