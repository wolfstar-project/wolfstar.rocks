...
yourapp running on docker



Once built, your app will say `'Hello undefined'` at localhost:3000 as it doesn’t have a way to access the environment variable yet. Let’s do that next.
...
```shell
docker build -t docker-nodejs . && docker run -e DOTENV_KEY="dotenv://: [\[email protected\]](https://www.dotenv.org/cdn-cgi/l/email-protection)/vault/.env.vault?environment=production" --rm -it -p 3000:3000 --init docker-nodejs
```

CopyCopied!

The important part here is the `-e` flag. See docker environment variables for more information.
