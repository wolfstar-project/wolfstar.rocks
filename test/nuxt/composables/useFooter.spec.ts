...
describe("useFooter", () => {
	...
	it("should have Product as the first column", () => {
		const { columns } = useFooter();
		expect(columns.value[0]!.label).toBe("Product");
	});

	it("should have Community as the second column", () => {
		const { columns } = useFooter();
	...
	it("should have 3 children in the Product column", () => {
		const { columns } = useFooter();
		expect(columns.value[0]!.children).toHaveLength(3);
	});

	it("should have 3 children in the Community column", () => {
		const { columns } = useFooter();
	...
	it("should include Support Server in Community", () => {
		const { columns } = useFooter();
		const labels = columns.value[1]!.children!.map((c: any) => c.label);
