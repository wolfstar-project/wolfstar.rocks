<template>
	<div class="space-y-6">
		...
		<GuildSettingsForm
			...
			class="space-y-6"
			:aria-label="ts('guild_settings.filter.capitals.form_aria')"
			@error="onError"
		>
			<!-- Section 1: Capital Letters Filter -->
			<GuildSettingsSection :title="ts('guild_settings.filter.capitals.title')">
				<div class="space-y-3">
					<UFormField name="selfmodCapitalsEnabled">
						<div class="flex items-start gap-3">
							<USwitch
								...
								class="mt-0.5"
								:aria-label="ts('guild_settings.filter.capitals.toggle')"
							/>
							<div>
								<p class="text-sm leading-none font-medium">
									{{
										ts("guild_settings.filter.capitals.filter_label", {
								...
								<p class="mt-1 text-xs text-muted">
									{{ ts("guild_settings.filter.capitals.toggle_help") }}
								</p>
							</div>
						</div>
					</UFormField>
			...
			<GuildSettingsSection :title="ts('guild_settings.filter.common.options')">
				<div class="space-y-5">
					<div>
						<p class="mb-2 text-sm font-medium">
							{{ ts("guild_settings.filter.capitals.minimum") }}
							<span class="ml-1 text-muted tabular-nums"
						...
						<USlider
							...
							:min="5"
							:max="2000"
							:aria-label="ts('guild_settings.filter.capitals.minimum')"
						/>
						<div class="mt-1 flex justify-between text-xs text-muted">
							<span>5</span>
							<span>2000</span>
					...
					<div>
						<p class="mb-2 text-sm font-medium">
							{{ ts("guild_settings.filter.capitals.maximum") }} (%)
							<span class="ml-1 text-muted tabular-nums"
						...
						<USlider
							...
							:min="10"
							:max="100"
							:aria-label="ts('guild_settings.filter.capitals.maximum')"
						/>
						<div class="mt-1 flex justify-between text-xs text-muted">
							<span>10%</span>
							<span>100%</span>
...
async function onError(event: FormErrorEvent) {
	...
	toast.add({
		color: "error",
		description: ts("guild_settings.filter.capitals.save_failed", {
