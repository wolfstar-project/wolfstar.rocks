...
export default defineNitroPlugin((nitroApp) => {
	...
	const drain = pipeline(async (batch) => {
		const tasks: Array<{ promise: Promise<unknown>; tag: string }> = [];
		...
		for (const [index, result] of results.entries()) {
			if (result.status === "rejected") {
				...
				console.warn(`[evlog] ${task?.tag ?? "unknown"} drain failed:`, result.reason);
			}
		}
	});
	...
	// Tamper-evident FS journal for audit events.
