...
The default memory cache is **lost on server restart**. For production, use a persistent storage driver like Redis or Cloudflare KV. Append `?purge` to any OG image URL to manually invalidate its cache entry.

::callout{icon="i-heroicons-arrow-right" to="/docs/og-image/guides/cache"}
See the [Caching guide](/docs/og-image/guides/cache) for storage drivers, build cache, cache keys, and TTL options.
::
...
- **Build cache**: persists across deploys ([Caching guide](/docs/og-image/guides/cache#build-cache-ci-persistence))
