...
- `forks` (the default) and `vmForks` run each file in a separate child process
- `threads` and `vmThreads` run each file in a separate worker thread
...
When tests are marked as `concurrent`, Vitest groups them together and runs them with `Promise.all`. The number of tests running at once is bounded by the [`maxConcurrency`](/config/maxconcurrency) option.
