	...
	<div class="w-full">
		...
		<div class="relative mx-auto min-h-100 w-full max-w-7xl">
			<div v-for="(feature, index) in features" :key="feature.id" class="animate-fade-in">
				<div v-if="activeFeature === index" class="items-center gap-8">
					<!-- Feature Details -->
										...
										>
											<template v-if="featureIndex === AutomodFeature.Attachments">
												...
												<div class="grid max-w-96 grid-cols-2 gap-2">
													<div
														class="flex aspect-video w-full items-center justify-center card-glass rounded-lg drop-shadow-lg"
													>
														<UIcon
...
/* Override Discord Embed Background */
:deep(.brand-embed .discord-embed) {
	backdrop-filter: blur(10px);
	border: 1px solid oklch(var(--color-branding-wolfstar) / 0.2);
	background-color: oklch(var(--color-branding-wolfstar) / 0.1) !important;
}
