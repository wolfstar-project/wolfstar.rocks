...
The module caches rendered images in Nitro's default cache storage with a 72-hour TTL, and emits immutable `Cache-Control` headers so CDNs can serve cached copies without revalidation:

- **First render**: 400-1500ms (Satori), 600-2500ms (Takumi), 1000-3500ms (Browser)
- **Cached render**: 5-30ms

Nitro's default storage is in-memory, which means the cache is **lost on server restart**. For production, point Nitro's `cache` storage at a persistent driver (for example Redis or Cloudflare KV) via `nitro.storage.cache`; the module picks it up automatically. Or use the module-specific `runtimeCacheStorage` option if you want OG images on a separate mount. Append `?purge` to any OG image URL to manually invalidate its cache entry.
