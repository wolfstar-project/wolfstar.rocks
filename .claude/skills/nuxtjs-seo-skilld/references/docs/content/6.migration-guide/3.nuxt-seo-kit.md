...
In v1, you had to prerender all pages; the module modified your `nuxt.config` to ensure this happened.

In v2, the module generates everything at runtime and no longer provides prerendering changes.
...
We've removed the following options from the nuxt.config `ogImage`:
...
- `OgImageBasic` - remove the property, so the module can select the fallback automatically
