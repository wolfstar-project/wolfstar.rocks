...
- `directive()` - Registers a global custom directive if passing both a name string and a directive definition, or retrieves an already registered one if only the name is passed[(example)](/docs/4.x/directory-structure/app/plugins#vue-directives).
- `use()` - Installs a **Vue.js Plugin** [(example)](/docs/4.x/directory-structure/app/plugins#vue-plugins).
...
- `event` (h3js/h3 request event) - Access the request & response of the current route.
...
#### Custom Reducer/Reviver
  ...
  Since Nuxt v3.4, it is possible to define your own reducer/reviver for types that are not supported by Nuxt.
