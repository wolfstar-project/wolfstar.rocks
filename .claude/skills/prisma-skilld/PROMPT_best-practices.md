Generate SKILL.md section for "prisma" v7.7.0.
...
| Types | `./references/pkg/./dist/cli/src/types.d.ts` — **read this file directly** to verify exports |
...
- `./references/pkg-prisma/` (1 .md files)
...
Skip: obvious API usage, installation steps, general TypeScript/programming patterns not specific to this package, anything a developer would naturally write without reading the docs. Every item must be specific to prisma — reject general programming advice that applies to any project.

Search: `skilld search "recommended" -p prisma`, `skilld search "avoid" -p prisma`

## Format

<format-example note="Illustrative structure only — replace placeholder names with real prisma APIs">
```
## Best Practices

- Use prisma's built-in `createX()`./references/docs/api.md#createx)
