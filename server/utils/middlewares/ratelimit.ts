 ...
 */
export function createRateLimit(options: Optional<RateLimitOptions> = {
    time: 5,
    max: 10,
    message: (event: H3Event) =>
      `Rate limit exceeded. Please try again later. IP address: ${getClientIP(event)}`,
    auth: false,
  }) {
  const {
    time,
