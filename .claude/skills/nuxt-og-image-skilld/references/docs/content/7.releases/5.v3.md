...
the rendering (Browser, Satori) and more.
...
With v3, the module now supports the `ogImage` frontmatter key for configuring your OG Image.
...
Whenever you add a template has a `<style>`{lang="html"} block, we need to use the css-inline dependency to make sure
...
The old pattern for OG images was `/<path>/__og_image__/og.png`, this has been changed to `/_og/d/<path>/og.<extension>`{lang="html"}.
...
:Image{src="/_og/d/og-image/releases/v3/og.png?component=Release&theme=%23d946ef&siteName=Purple&icon=carbon%3Afire" label="Theming" alt="v3 dark mode"}
:Image{src="/_og/d/og-image/releases/v3/og.png?component=Release&colorMode=light" label="Light mode" alt="v3 light mode"}
:Image{src="/_og/d/og-image/releases/v3/og.png?component=Release&colorMode=dark" label="Dark mode" alt="v3 dark mode"}

::

Learn more on the [NuxtSeo Template](/docs/og-image/guides/community-templates) docs.
...
Stale-While-Revalidate is now enabled by default, it also will correctly respond with 304 Not Modified
