...
`tracesSampleRate` SDK option, like
...
The `instrumentation.ts` file is a Next.js native API called instrumentation hook.

To start using the Next.js instrumentation hook, follow these steps:

1. First, enable the Next.js instrumentation hook by setting the `experimental.instrumentationHook` to true in your `next.config.js`. (This step is no longer required with Next.js 15)
...
TypeScript definitions of `componentDidCatch` for the
...
You can use the **Experimental** @sentry/migr8 to automatically update
