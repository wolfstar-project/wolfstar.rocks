...
Skip: obvious API usage, installation steps, general TypeScript/programming patterns not specific to this package, anything a developer would naturally write without reading the docs. Every item must be specific to @playwright/test — reject general programming advice that applies to any project.

Search: `skilld search "recommended" -p @playwright/test`, `skilld search "avoid" -p @playwright/test`

## Format

<format-example note="Illustrative structure only — replace placeholder names with real @playwright/test APIs">
```
## Best Practices

- Use @playwright/test's built-in `createX()`./references/docs/api.md#createx)
