<template>
  <div
    class="relative flex h-full flex-col rounded-xl border border-base-300/70 bg-base-100 p-2 md:p-4 shadow-lg transition-all duration-300 ease-out transform-gpu motion-reduce:transform-none motion-reduce:transition-none"
    :class="cardClasses"
  >
    ...
    <template v-if="!loading && guild">
      ...
      <div class="flex md:hidden flex-col items-center gap-2">
        ...
        >
          ...
          <div class="absolute inset-0 flex items-center justify-center rounded-full bg-success/80 opacity-0 transition-opacity group-hover:opacity-100 group-active:opacity-100">
            <UIcon name="heroicons:adjustments-horizontal" class="size-5 text-success-content" aria-hidden="true" />
          </div>
        </NuxtLink>
        <NuxtLink
        ...
        >
          <guild-icon :guild variant="bare" size="lg" :show-status="true" />
          <div class="absolute inset-0 flex items-center justify-center rounded-full bg-primary/80 opacity-0 transition-opacity group-hover:opacity-100 group-active:opacity-100">
            <UIcon name="heroicons:rocket-launch" class="size-5 text-primary-content" aria-hidden="true" />
          </div>
        </NuxtLink>
        <div
          v-else
      ...
      <div class="hidden md:flex h-full w-full flex-col items-center gap-3 text-center">
