...
This stores rendered images in `node_modules/.cache/nuxt-seo/og-image/` during prerendering (when using pnpm, this will be in the `node_modules/.pnpm` directory's cache structure; consider using a custom cache directory for consistency). The cache automatically invalidates when:
...
Runtime OG image URLs encode all options in the path. Passing many props or long values can produce long URLs. See the [Performance](/docs/og-image/guides/performance) guide for strategies.
