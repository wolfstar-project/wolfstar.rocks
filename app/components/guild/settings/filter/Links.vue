<template>
	<div class="space-y-6">
		...
		<GuildSettingsForm
			...
			class="space-y-6"
			:aria-label="ts('guild_settings.filter.links.form_aria')"
			@error="onError"
		>
			<GuildSettingsSection :title="ts('guild_settings.filter.links.title')">
				<div class="space-y-3">
					<UFormField name="selfmodLinksEnabled">
						<div class="flex items-start gap-3">
							<USwitch
								...
								class="mt-0.5"
								:aria-label="ts('guild_settings.filter.links.toggle')"
							/>
							<div>
								<p class="text-sm leading-none font-medium">
									{{
										ts("guild_settings.filter.links.filter_label", {
								...
								<p class="mt-1 text-xs text-muted">
									{{ ts("guild_settings.filter.links.toggle_help") }}
								</p>
							</div>
						</div>
					</UFormField>
			...
			<GuildSettingsSection :title="ts('guild_settings.filter.links.allowed_links')">
				<div class="space-y-4">
					<form class="flex gap-2" @submit.prevent="addLink">
						<UInput
							v-model="newLink"
							:placeholder="ts('guild_settings.filter.links.add_link_placeholder')"
							class="w-64"
							:aria-label="ts('guild_settings.filter.links.add_link')"
						/>
					...
					<div v-if="state.selfmodLinksAllowed.length > 0" class="flex flex-wrap gap-2">
						...
						>
							...
							<button
								...
								:aria-label="
									ts('guild_settings.filter.links.remove_link', { link })
								"
...
async function onError(event: FormErrorEvent) {
	...
	toast.add({
		color: "error",
		description: ts("guild_settings.filter.links.save_failed", {
