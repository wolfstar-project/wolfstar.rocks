...
Nuxt OG Image supports emoji rendering across all renderers using Iconify emoji sets. The module detects emoji characters and replaces them with inline SVGs at render time.
...
::note
The module supports only ~100 common emojis by default. For additional emojis, use Nuxt Icon with the emoji icon set directly.
::
...
When using the [Takumi](/docs/og-image/renderers/takumi) renderer, you can use COLR emoji fonts (e.g., Twemoji) directly as a font instead of SVG replacement. This is faster and produces higher quality emoji rendering.
