...
If you want OG images on a separate mount from the rest of your Nitro cache, use `runtimeCacheStorage`, which accepts the same configuration as Nitro's `storage` option.

### Using Runtime Config

If you need environment-specific configuration (e.g., different Redis hosts per environment), mount your own storage driver and reference it by key:
...
- **Vercel**: Redeploy, or use the Vercel CLI to purge
